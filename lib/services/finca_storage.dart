...
// TODO: Este archivo contiene datos de prueba locales que no coinciden con el backend.
// Por ahora está deshabilitado. Se recomienda usar finca_service.dart para consultar el backend.
// Si se necesita almacenamiento local, adaptar a la estructura del backend.
...
class FincaStorage {
  ...
  void inicializarFincasEjemplo() {
    // TODO: Adaptar al formato del backend cuando sea necesario
  ...
  List<Finca> buscarFincas(String termino) {
    ...
    return _todasLasFincas.where((finca) {
      return finca.nombre.toLowerCase().contains(terminoLower) ||
  ...
  List<Finca> filtrarFincas({double? precioMaximo}) {
    return _todasLasFincas.where((finca) {
      // Filtro por precio máximo
      if (precioMaximo != null && finca.precioPorNoche > precioMaximo) {
        return false;
      }
      return true;
    }).toList();
  ...
  void eliminarMiFinca(String fincaId) {
    ...
    _todasLasFincas.removeWhere(
      (finca) =>
          finca.id == fincaId && finca.propietario?['id'] == _usuarioActualId,
    );
  }

  // Funciones de ejemplo comentadas hasta que se necesiten
  /*
