  ...
  Future<void> _guardarFinca() async {
    ...
    try {
      ...
      if (_fotos.isNotEmpty) {
        print('📸 Subiendo ${_fotos.length} imágenes...');

        for (int i = 0; i < _fotos.length; i++) {
          try {
            final foto = _fotos[i];
            print('  Procesando imagen ${i + 1}/${_fotos.length}...');

            // Leer los bytes de la imagen
            final Uint8List imageBytes = await foto.readAsBytes();

            // Convertir a base64
            final String base64Image = base64Encode(imageBytes);
            final String imageBase64 = 'data:image/jpeg;base64,$base64Image';

            print('  Tamaño imagen: ${imageBase64.length} caracteres');

            // Subir la imagen al backend
            final success = await _fincaService.agregarImagenAFinca(
              fincaId: int.parse(nuevaFinca.id),
            ...
            if (success) {
              ...
              print('  ⚠️ Error al subir imagen ${i + 1}');
            }
          } catch (e) {
        ...
        print('ℹ️ No se seleccionaron imágenes');
      }
